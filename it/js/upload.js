...
function showOutput(files) {
    ...
    switch (files.length) {
        ...
        case 2:
            ...
            if (type == type2) {
                output.innerHTML = "<span style='color: red;'>non servono 2 " + type + "</span>";
                return;
            }
            if (type2 == "css")
                cssFile = files[1];
        ...
        default:
            output.innerHTML = "<span style='color: red;'>puoi caricare al massimo 1 css ed 1 .woff </span>";
            return;
    ...
    if (cssFile == null) {
        if (woffFile != null)
            output.innerHTML = woffFile.name + " caricato ma <br> <span style='color: red;'> devi caricare anche un file .css</span>";
        else
...
function preCheck(data) {
    ...
    if (uploadEnabled) {
        if (getCssInfo()) {
            ...
            p.innerHTML = "Trovato altro font con lo stesso nome: " + oldName + ". <span style='font-family:" + oldName + ";'> Esempio </span>";
            changeRename();
        }
        else {
            var div = document.getElementById("rename").parentNode;
...
function checkCss(preCheckError) {
    ...
    switch (preCheckError) {
        case -1:
            ...
            if (woffFile != null)
                msg = msg + "Il file woff non serve: la src del font è online";
            setUploadEnabled(true);
            break;
        case 0:
            msg = "<span style='color: red;'> css non valido: comando @font-face non trovato</span>";
            setUploadEnabled(false);
            break;
        case 1:
            msg = "<span style='color: red;'> css non valido: font-family non trovata </span>";
            setUploadEnabled(false);
            break;
        case 2:
            msg = "<span style='color: red;'> css non valido: src non trovata </span>";
            setUploadEnabled(false);
            break;
        case 3:
            msg = "<span style='color: red;'> css non valido: url non trovata </span>";
            setUploadEnabled(false);
            break;
        case 4:
...
function checkWoff() {
    ...
    if (woffFile != null) {
        ...
        msg += "Serve un file .woff / .woff2 " + "<span style='color: red;'>&#10006</span><br>";
